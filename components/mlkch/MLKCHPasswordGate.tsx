...
// Hardcoded access code — swap for env variable or API check in a real auth flow
const ACCESS_CODE = 'mlkch8770';
const SESSION_KEY = 'mlkch_auth';

interface MLKCHPasswordGateProps {
  children: React.ReactNode;
}

const MLKCHPasswordGate: React.FC<MLKCHPasswordGateProps> = ({ children }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative w-full max-w-sm rounded-3xl px-6 py-5 ${shaking ? 'animate-[mlkch-shake_0.4s_ease]' : ''}`}
        style={{
          background:
...
export default MLKCHPasswordGate;
